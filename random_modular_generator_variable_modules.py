...
def create_total_degree_sequence (n, sfunction, avg_degree, mod_nodes, tolerance, max_tries=2000, **kwds):
    ...
    for mod in range(len(mod_nodes.keys())):
        ...
        while tol > tolerance  or (not is_valid_seq) or (tries > max_tries):
            ...
            # is_valid_degree_sequence is changed by urkang (191117)
            is_valid_seq = nx.is_valid_degree_sequence_erdos_gallai(seq)
...
def test_modularity_variable_mod(G, mod_nodes):
    ...
    for modules in range (0,mods):
        ...
        for node in mod_nodes[modules]:
            aii=[n for n in G.neighbors(node)]# total degree of a node, modified by urkang, 191117
...
if __name__ == "__main__":
    ...
    try :
        ...
        # csv matrix save added by urkang, 191117
        print ("Saving adjacency matrix under the filename adjacency_matrix.txt & adjacency_matrix.csv")
